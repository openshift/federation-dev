...
usage()
{
  ...
  echo "-s|--steps - Accepted values: [context-creation|setup-gogs|load-git-content|demo-only]."
...
get_data_from_inventory()
{
  INVENTORY=$1
  download_jq_binary
  HUB_URL=$(./bin/jq -r '.hub.url' $INVENTORY)
  HUB_VERSION=$(./bin/jq -r '.hub.ocp_version' $INVENTORY)
  ...
  HUB_API_URL="https://api.${HUB_URL}:6443"
  ...
  if [ "0$HUB_VERSION" == "03" ]
  then
    HUB_API_URL="https://console.${HUB_URL}:8443"
  fi
  ...
  HUB_ADMIN=$(./bin/jq -r '.hub.admin_user' $INVENTORY)
  HUB_ADMIN_PWD=$(./bin/jq -r '.hub.admin_password' $INVENTORY)
  ...
  echo "  HUB Domain: ${HUB_URL} - API: ${HUB_API_URL} - Admin User: ${HUB_ADMIN}"
...
{
  NS_NAME="$1"
  NAMESPACE_CHECK=$(oc --context=feddemohub get ns ${NS_NAME} -o name 2>/dev/null | wc -l)
  NAMESPACE_ALREADY_EXISTS=0
  if [ "0${NAMESPACE_CHECK}" == "01" ]
  then
    NAMESPACE_ALREADY_EXISTS=1
  else
    run_ok_or_fail "oc --context=feddemohub create ns ${NS_NAME}" "0" "1"
    run_ok_or_fail "oc --context=feddemohub annotate namespace ${NS_NAME} auto-demo='${DEMO_NAMESPACE}'" "0" "1"
  fi
...
{
  for cluster in feddemohub feddemocl1 feddemocl2 feddemocl3
  do
    oc --context=$cluster -n default create route edge wildcarddomain --service=test --port=8080 > /dev/null
  done
  WILDCARD_DOMAIN_HUB=$(oc --context=feddemohub -n default get route wildcarddomain -o jsonpath='{.status.ingress[*].host}' | sed "s/wildcarddomain-default.\(.*\)/\1/g")
...
check_acm_deployment()
{
  NAMESPACE_CHECK=$(oc --context=feddemohub get ns ${ACM_NAMESPACE} -o name 2>/dev/null | wc -l)
  if [ "0${NAMESPACE_CHECK}" == "01" ]
  then
    MANAGED_CLUSTERS=$(oc --context=feddemohub get clusters -A -o name 2>/dev/null | wc -l)
    if [ "0$MANAGED_CLUSTERS" -lt "03" ]
    then
      echo "Less than 3 managed clusters. Exiting..."
    fi
  else
    echo "ACM Namespace not detected. Exiting..."
    exit 1
  fi
}
...
setup_haproxy()
{
  ...
  echo "Deploying HAProxy on feddemohub cluster in namespace ${HAPROXY_NAMESPACE}"
  HAPROXY_LB_ROUTE=pacman-multicluster.${WILDCARD_DOMAIN_HUB}
  PACMAN_INGRESS=pacman-ingress.${WILDCARD_DOMAIN_HUB}
  run_ok_or_fail "oc --context=feddemohub -n ${HAPROXY_NAMESPACE} create route edge haproxy-lb --service=haproxy-lb-service --port=8080 --insecure-policy=Allow --hostname=${HAPROXY_LB_ROUTE}" "0" "1"
  ...
  run_ok_or_fail "oc --context=feddemohub -n ${HAPROXY_NAMESPACE} create configmap haproxy --from-file=yaml-resources/haproxy/haproxy"
  run_ok_or_fail "oc --context=feddemohub -n ${HAPROXY_NAMESPACE} create -f yaml-resources/haproxy/haproxy-clusterip-service.yaml"
  run_ok_or_fail "oc --context=feddemohub -n ${HAPROXY_NAMESPACE} create -f yaml-resources/haproxy/haproxy-deployment.yaml"
...
setup_gogs()
{
  ...
  echo "Deploying Gogs Git Server on feddemohub cluster in namespace ${GOGS_NAMESPACE}"
  GOGS_ROUTE=gogs-demo.${WILDCARD_DOMAIN_HUB}
  run_ok_or_fail "oc --context feddemohub -n ${GOGS_NAMESPACE} apply -f yaml-resources/gogs/postgres.yaml"
  wait_for_deployment_ready "feddemohub" "${GOGS_NAMESPACE}" "postgres" "0"
  run_ok_or_fail 'cat yaml-resources/gogs/gogs.yaml | sed "s/changeMe/${GOGS_ROUTE}/g" | oc --context feddemohub -n ${GOGS_NAMESPACE} apply -f -' "0" "1"
  wait_for_deployment_ready "feddemohub" "${GOGS_NAMESPACE}" "gogs" "0"
  echo "Initializing Gogs User"
  GOGS_POD=$(oc --context feddemohub -n ${GOGS_NAMESPACE} get pod -l name=gogs -o jsonpath='{.items[*].metadata.name}')
  run_ok_or_fail 'oc --context feddemohub -n ${GOGS_NAMESPACE} exec ${GOGS_POD} init-gogs "demouser" "demouser" "demouser@d3m0.com"' "0" "1"
...
load_git_content()
{
  ...
  mkdir -p mongo/base mongo/overlays/cluster1 mongo/overlays/cluster2 mongo/overlays/cluster3 pacman/base pacman/overlays/cluster1 pacman/overlays/cluster2 pacman/overlays/cluster3 acm/manifests &> /dev/null
  ...
  cp -pf ../yaml-resources/acm/git-channel-mod.yaml acm/manifests/git-channel.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/application-pacman.yaml acm/manifests/application-pacman.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/placementrule-cluster1.yaml acm/manifests/placementrule-cluster1.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/placementrule-cluster2.yaml acm/manifests/placementrule-cluster2.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/placementrule-cluster3.yaml acm/manifests/placementrule-cluster3.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-mongo-cluster1-mod.yaml acm/manifests/subscription-mongo-cluster1.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-mongo-cluster2-mod.yaml acm/manifests/subscription-mongo-cluster2.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-mongo-cluster3-mod.yaml acm/manifests/subscription-mongo-cluster3.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-pacman-cluster1-mod.yaml acm/manifests/subscription-pacman-cluster1.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-pacman-cluster2-mod.yaml acm/manifests/subscription-pacman-cluster2.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-pacman-cluster3-mod.yaml acm/manifests/subscription-pacman-cluster3.yaml &> /dev/null
...
setup_mongo_tls()
{
  ...
  cp -pf ../yaml-resources/acm/git-channel.yaml ../yaml-resources/acm/git-channel-mod.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-mongo-cluster1.yaml ../yaml-resources/acm/subscription-mongo-cluster1-mod.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-mongo-cluster2.yaml ../yaml-resources/acm/subscription-mongo-cluster2-mod.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-mongo-cluster3.yaml ../yaml-resources/acm/subscription-mongo-cluster3-mod.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-pacman-cluster1.yaml ../yaml-resources/acm/subscription-pacman-cluster1-mod.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-pacman-cluster2.yaml ../yaml-resources/acm/subscription-pacman-cluster2-mod.yaml &> /dev/null
  cp -pf ../yaml-resources/acm/subscription-pacman-cluster3.yaml ../yaml-resources/acm/subscription-pacman-cluster3-mod.yaml &> /dev/null
  ...
  echo "Crafting ACM Manifest for mongodb and pacman"
  run_ok_or_fail 'sed -i "s|gitreporoute|http://$GOGS_ROUTE/demouser/gitops-demo.git|" ../yaml-resources/acm/git-channel-mod.yaml' "0" "1"
  run_ok_or_fail 'sed -i "s/demoNS/${DEMO_NAMESPACE}/" ../yaml-resources/acm/subscription-mongo-cluster1-mod.yaml' "0" "1"
  run_ok_or_fail 'sed -i "s/demoNS/${DEMO_NAMESPACE}/" ../yaml-resources/acm/subscription-mongo-cluster2-mod.yaml' "0" "1"
  run_ok_or_fail 'sed -i "s/demoNS/${DEMO_NAMESPACE}/" ../yaml-resources/acm/subscription-mongo-cluster3-mod.yaml' "0" "1"
  run_ok_or_fail 'sed -i "s/demoNS/${DEMO_NAMESPACE}/" ../yaml-resources/acm/subscription-pacman-cluster1-mod.yaml' "0" "1"
  run_ok_or_fail 'sed -i "s/demoNS/${DEMO_NAMESPACE}/" ../yaml-resources/acm/subscription-pacman-cluster2-mod.yaml' "0" "1"
  run_ok_or_fail 'sed -i "s/demoNS/${DEMO_NAMESPACE}/" ../yaml-resources/acm/subscription-pacman-cluster3-mod.yaml' "0" "1"
  cd .. &> /dev/null
}
...
mongo_pacman_demo_cleanup()
{
  ...
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-pacman-cluster1.yaml" "0" "1" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-pacman-cluster2.yaml" "0" "1" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-pacman-cluster3.yaml" "0" "1" "1"
  echo "Deleting MongoDB resources"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster1.yaml" "0" "1" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster2.yaml" "0" "1" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster3.yaml" "0" "1" "1"
  wait_for_pod_not_running "feddemocl1" "${DEMO_NAMESPACE}" "mongo"
  wait_for_pod_not_running "feddemocl2" "${DEMO_NAMESPACE}" "mongo"
  wait_for_pod_not_running "feddemocl3" "${DEMO_NAMESPACE}" "mongo"
  wait_for_pod_not_running "feddemocl1" "${DEMO_NAMESPACE}" "pacman"
  wait_for_pod_not_running "feddemocl2" "${DEMO_NAMESPACE}" "pacman"
  wait_for_pod_not_running "feddemocl3" "${DEMO_NAMESPACE}" "pacman"
  DELETE_NS=$(delete_annotated_namespace feddemohub ${DEMO_NAMESPACE})
  if [ "0${DELETE_NS}" == "01" ]
  then
    run_ok_or_fail "oc --context=feddemohub delete namespace ${DEMO_NAMESPACE}" "0" "1" "1"
  fi
}
...
haproxy_cleanup()
{
  ...
  run_ok_or_fail "oc --context=feddemohub delete namespace ${HAPROXY_NAMESPACE}" "0" "1"
}
...
gogs_cleanup()
{
  ...
  DELETE_NS=$(delete_annotated_namespace feddemohub ${GOGS_NAMESPACE})
  if [ "0${DELETE_NS}" == "01" ]
  then
    run_ok_or_fail "oc --context=feddemohub delete namespace ${GOGS_NAMESPACE}" "0" "1" "1"
  fi
}
...
mongo_pacman_demo()
{
  # TODO: To be updated to use 1 placementrule and 1 sub when this rfe is fullfilled https://github.com/open-cluster-management/backlog/issues/1669
  PACMAN_URL=pacman-multicluster.${WILDCARD_DOMAIN_HUB}
  GOGS_ROUTE=gogs-demo.${WILDCARD_DOMAIN_HUB}
  DEMO_NAMESPACE_ALREADY_EXISTS=$(create_annotated_namespace $DEMO_NAMESPACE)
  if [ "0${DEMO_NAMESPACE_ALREADY_EXISTS}" == "01" ]
  then
    echo "${DEMO_NAMESPACE} namespace already exists in the cluster. We cannot proceed with the demo."
    exit 1
  else
    echo "Namespace ${DEMO_NAMESPACE} created for deploying the demo."
  fi
  echo "1. Let's start by configuring our Git repository into ACM, we will use a Channel for that."
  # TODO
  # 
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/git-channel.yaml" "0" "1"
  echo "2. Now, we need our MongoDB replicas running across three clusters, we are going to create a PlacementRule targetting 3 clusters from all healthy managed clusters."
  # TODO
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/placementrule-cluster1.yaml" "0" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/placementrule-cluster2.yaml" "0" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/placementrule-cluster3.yaml" "0" "1"
  echo "2. Now it is time to deploy our MongoDB Subscriptions so we will get a MongoDB replica running on each cluster."
  # TODO
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster1.yaml" "0" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster2.yaml" "0" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster3.yaml" "0" "1"
  ...
  echo "10. Now it is time to create our Pacman subscriptions so we will get a Pacman replica running on each cluster."
  wait_for_input
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-pacman-cluster1.yaml" "0" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-pacman-cluster2.yaml" "0" "1"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-pacman-cluster3.yaml" "0" "1"
  ...
  echo "17. Now we will create the application for visualizing the app on the WebUI"
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/application-pacman.yaml" "0" "1"
  echo "18. Go play Pacman and save some highscores. http://${PACMAN_URL} (Note: Pretend you're bad at Pacman)."
  wait_for_input
  if [ $CI_MODE -eq 1 ]
  then
  ...
  echo "19. Well, everything should be working fine. Let's create some chaos, what will happen if primary mongo pod gets deleted?"
  wait_for_input
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} delete -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster1.yaml" "0" "1"
  wait_for_pod_not_running "feddemocl1" "${DEMO_NAMESPACE}" "mongo"
  run_ok_or_fail "oc --context=feddemocl1 -n ${DEMO_NAMESPACE} get pods --selector='name=mongo'" "1" "1"
  echo "20. Let's continue playing and see if we can save highscores. http://${PACMAN_URL}. (Note: Saving the high score could take longer than usual)."
  wait_for_input
  if [ $CI_MODE -eq 1 ]
  then
  ...
  echo "21. Well, our Pacman application is not that famous, let's scale it so it only runs on one of our clusters."
  pacman_scale "cluster1" "0"
  pacman_scale "cluster3" "0"
  echo "Here we will implement a wait function, press any key to continue"
  read
  run_ok_or_fail 'for cluster in feddemocl1 feddemocl2 feddemocl3;do echo **Cluster ${cluster}**;oc --context=$cluster -n ${DEMO_NAMESPACE} get pods --selector="name=pacman";done' "1" "1"
  echo "22. Our engineers have been working hard during the weekend and the cluster where the primary mongo pod was deployed came back to life."
  wait_for_input
  run_ok_or_fail "oc --context=feddemohub -n ${DEMO_NAMESPACE} create -f http://$GOGS_ROUTE/demouser/gitops-demo/raw/master/acm/manifests/subscription-mongo-cluster1.yaml" "0" "1"
  wait_for_deployment_ready "feddemocl1" "${DEMO_NAMESPACE}" "mongo" "0"
  run_ok_or_fail "oc --context=feddemocl1 -n ${DEMO_NAMESPACE} get pods --selector='name=mongo'" "1" "1"
  echo "23. Our Pacman application has become trendy among teenagers, they don't want to play Fortnite anymore. We need to scale!!"
  wait_for_input
  ...
  wait_for_deployment_ready "feddemocl3" "${DEMO_NAMESPACE}" "pacman" "0"
  run_ok_or_fail 'for cluster in feddemocl1 feddemocl2 feddemocl3;do echo **Cluster ${cluster}**;oc --context=$cluster -n ${DEMO_NAMESPACE} get pods --selector="name=pacman";done' "1" "1"
  echo "24. Bonus track: We should see our MongoDB Replica being restored."
  wait_for_deployment_ready "feddemocl1" "${DEMO_NAMESPACE}" "mongo" "0"
  wait_for_mongodb_replicaset "feddemocl1" "${DEMO_NAMESPACE}" "3"
  if [ $CI_MODE -eq 1 ]
  then
...
check_steps()
{
  ...
  do
    ...
    then
      ...
      echo "Invalid step $STEP_TO_CHECK, valid steps: [context-creation|setup-gogs|setup-haproxy|load-git-content|demo-only]"
      exit 1
    fi
  done
}
...
main()
{
  ...
  then
    ...
    then
      echo "We are going to create four contexts into our oc tool config"
      context_creation "feddemohub" ${HUB_API_URL} ${HUB_ADMIN} ${HUB_ADMIN_PWD} 
    ...
    if [ $RUN_ACM_CHECK -eq 1 ]
    then
      echo -ne "\n\n\nValidating ACM installation\n"
      check_acm_deployment
    fi
...
MODE=${MODE:="demo"}
CI_MODE=${CI_MODE:="0"}
DEMO_NAMESPACE=${DEMO_NAMESPACE:="acm-gitops-auto-demo"}
main
