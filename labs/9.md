...
The values of the MongoDB replicas needs to be defined within the *pacman* deployment YAML.
~~~sh
...
~~~sh
argocd app create --project default --name rhte-pacman-cluster1 --repo https://github.com/openshift/federation-dev.git --path labs/lab-6-assets --dest-server https://kubernetes.default.svc  --dest-namespace pacman  --revision argocd  --sync-policy none
~~~

Now that the cluster1 has the application defined the other two clusters need to be defined as well.
~~~sh
argocd app create --project default --name rhte-pacman-cluster2 --repo https://github.com/openshift/federation-dev.git --path labs/lab-6-assets --dest-server https://api.east-2.sysdeseng.com:6443  --dest-namespace pacman  --revision argocd  --sync-policy none
argocd app create --project default --name rhte-pacman --repo https://github.com/openshift/federation-dev.git --path labs/lab-6-assets --dest-server https://api.west-2.sysdeseng.com:6443 --dest-namespace pacman  --revision argocd  --sync-policy none
~~~
