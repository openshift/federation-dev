...
The lab content provides the required files for deploying the federated MongoDB ReplicaSet in YAML format (similar to the previous lab). We will 
manually create the secret on each cluster but the persistent storage, deployment, and service will be created by ArgoCD.
...
1. Configure `MongoDB's PEM`
    ...
    ~~~sh
    sed -i "s/mongodb.pem: .*$/mongodb.pem: $(openssl base64 -A < mongo.pem)/" 01-secret.yaml
    sed -i "s/ca.pem: .*$/ca.pem: $(openssl base64 -A < ca.pem)/" 01-secret.yaml
    ~~~
